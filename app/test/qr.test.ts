...
import { scanStep } from "../src/wc/qr-loop.ts";
import { firstAccepted, QR_UNAVAILABLE, qrUnavailable, qrScanningAvailable } from "../src/wc/qr-loop.ts";
...
group("the scan loop survives frames that decode nothing");
{
  ...
  check(await scanStep(() => undefined, () => results++, () => errors++) === "continue",
  ...
  check(await scanStep(() => "wc:abc", (v) => { results++; check(v === "wc:abc", "wrong value passed on"); }, () => errors++) === "done",
  ...
  check(await scanStep(() => { throw new Error("camera died"); }, () => results++, (m) => { errors++; check(m === "camera died", `error text lost: ${m}`); }) === "done",
