 ...
 *
 * ---------------------------------------------------------------------------
 * Scope: C2 of docs/apps/ARC-LA-CAJA.md, and nothing past it
 *
 * A waiter enters a total, picks a tip, and gets a QR and a link. It ends at
 * "a customer can scan this and pay". There is no payment watcher (C3), no
 * shift grant or staff accounting (C4), no CajaInbox, relayer or CCTP (C5) and
 * no CajaTill (C6). In particular the QR pays the merchant address the shell is
 * showing; C5 replaces that with a per-chain CajaInbox whose destination is
 * immutable, and the only line that changes is the recipient handed to
 * `buildPaymentUri`.
 *
 * ---------------------------------------------------------------------------
...
import { newMarker, parseCents, TIP_PRESETS, type Cents } from "./order.ts";
...
export const TILL_APP: MiniApp = {
  ...
  async mount(root: HTMLElement, context: AppContext) {
    ...
    const redraw = () => {
      ...
      renderCharge(output, tillView(state), (chainId) => {
