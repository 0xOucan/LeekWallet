...
| `deployEquity` selector | `0x837b37b6` | **corrected 2026-09-10.** This row read `0x29002951` until then; that is `deployBond`. Both were re-derived with `cast sig` from the canonical signatures and confirmed by ABI-decoding live calldata of each shape — see `app/packages/apps/ats/contracts/RUNBOOK.md` §0 |
...
# SecurityData.resolver is the first field of the first struct
```
