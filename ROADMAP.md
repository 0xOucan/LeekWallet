...
| T65 | Firmware flasher in the companion app | T11 | `espflash` as a library rather than a bundled CLI. **Android is nearly free and was mis-scoped once**: T59 already ships USB serial on the phone, and `android-usb-serial` carries a `serialport_compat` module whose `SerialPortAdapter` implements the whole `serialport::SerialPort` trait — including `write_data_terminal_ready` and `write_request_to_send`, which are exactly the two calls that put the chip in download mode. So the chain is adapter → `espflash::Connection` → image, and the phone is not a second implementation. **Gated on T11 in substance, not just in sequence**: without secure boot a flasher is a one-click way to install firmware that captures the PIN, so a device that is not locked must say so plainly rather than the button being quietly greyed |
