...
| T65 | Firmware flasher in the companion app | T11 | **Built on `feat/flasher`, inert by default.** `espflash` is a library dependency behind a `flasher` cargo feature that shipped builds do not enable, so `flash_capability()` reports false and the button is disabled with the reason on screen. The image checks compile unconditionally and are tested either way: the write lands at 0x0 or at the app partition and nowhere between (a partial write over the partition table destroys wallets on a device that still boots), the chip is compared against the image's own id before any erase, and the SHA-256 is recomputed in Rust over the exact buffer handed to espflash — the frontend's digest cannot speak for bytes that crossed an IPC bridge. A mismatch refuses rather than warns. **Desktop only, and the earlier note here was wrong twice.** It first said Android needed a new `UsbManager` plugin; T59 had already shipped one. It then said Android was nearly free through `android-usb-serial`'s `serialport_compat`, whose `SerialPortAdapter` does implement the whole `serialport::SerialPort` trait including the DTR/RTS calls. That half is true and does not help: `espflash::connection::Port` is a concrete `serialport::TTYPort`/`COMPort` alias taken **by value**, not a `Box<dyn SerialPort>`, so there is no seam to hand an adapter through. The phone needs a change in espflash, a fork, or our own esptool implementation. Still needed besides that: a signed release manifest, so there is something safe to install. **Gated on T11 in substance, not just in sequence**: without secure boot a flasher is a one-click way to install firmware that captures the PIN, so a device that is not locked must say so plainly rather than the button being quietly greyed |
...
| ~~T67b~~ ✅ | Chrome extension that dapps see as a wallet | — | An injected **EIP-1193** provider announced over **EIP-6963**, the way KeepKey bridges its device and Rabby offers to take MetaMask's place. Reaches every EVM dapp including injected-only ones, with **no WalletConnect relay** and no QR — the relay currently sees that a wallet and a dapp are talking, and this removes that observer entirely. Cheap because `app/packages/core` is already transport-agnostic: `Transport` is `send(frame)` + `onFrame(handler)` and nothing else, so the extension inherits CBOR, framing, the X25519 session, EIP-712 rendering, `eth-decode`, the approval cap and the simulation unchanged. **The MV3 wrinkle is the real work**: a service worker is ephemeral and cannot hold a serial port, so the connection lives in an offscreen document. Device link is Web Serial or WebUSB from an extension page — desktop Chromium only, since Chrome ships no extensions on Android and Firefox implements neither API. Announce over 6963 rather than clobbering `window.ethereum`; offering to override is Rabby's separate, explicit opt-in and should stay separate here too. **Done, verified on hardware**: a live `app.aave.com` session signed and broadcast from the device over USB, announced over EIP-6963 beside other wallets. The offscreen document was never the problem. Three faults wore one error message. **The wire format was wrong**: the device frames USB as `'L' 'K' \| len:u16 \| type` and BLE as `len:u16 \| type`, and `device-client.ts` imported the BLE framing from `core/framing.ts` — so it read two bytes of IDF console log as a big-endian length and reported `invalid frame length 18720`, which is `0x4920`, ASCII `"I "` from `I (1234) tag:`. On a clean line it would have read `'L','K'` as 19531 and failed identically. Nothing caught it in four months because the desktop companion reaches serial through Rust, where `wire.rs` has carried the marker and the resynchronisation from the start; the extension is the only TypeScript that speaks USB. **The send was unbounded**: the deadline was armed after `transport.send()` resolved, and Web Serial's `writer.write()` resolves on stream backpressure rather than delivery, so one pending write wedged the serialised queue for the life of the client — "Connecting…" with no error and nothing recovering, the status poll included. **And the settle window was a guess**: `port.open()` raises DTR, which on USB-Serial-JTAG is wired to reset, so the handshake was arriving in a chip that was still booting; it now waits for the boot log to go quiet rather than for a fixed 250 ms copied from a problem where `dtr_on_open(false)` had prevented the reset outright. **The reboot itself is unavoidable** — Web Serial cannot suppress the toggle — so the PIN is re-entered on every connect, and that is recorded as a limitation rather than left as folklore |
| T68 | Web flasher on the website | T65 | **Done** in the website repository — esptool-js over Web Serial, SHA-256 shown before writing and a mismatch refusing rather than warning. Listed here because it shares T65's gate in substance: once secure boot exists, the flasher must say whether the image it is about to write is signed |
...
| T58c | PSRAM is present and deliberately off **except on the CAM board**, where `sdkconfig.s3cam` turns it on in octal mode because the camera framebuffers and quirc's image and pixel planes are ~310 KB together and cannot fit beside the wallet in 512 KB of SRAM. `CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL=16384` keeps every allocation this firmware makes on-die and sends only those huge image buffers outward; none of them holds key material, and static and stack storage is internal by construction. Still short of the goal below: the guarantee is a threshold, not `MALLOC_CAP_INTERNAL` on each secret, and external-memory encryption still waits on T11. Original entry: `esptool` reports 8 MB embedded on the attached board; the config had disabled it under a comment describing a Mini that has none. External memory is a separate die, and a seed the allocator spills there is reachable in ways internal SRAM is not — with the S3's external-memory encryption riding on T11. Enable only when something needs the space, with secrets pinned `MALLOC_CAP_INTERNAL`. Octal PSRAM also takes GPIO35/36/37 | T11 | enabled with a test that no secret allocation lands off-die |
...
| ~~T70~~ ✅ | **Physical dice entropy** at seed creation, counted in bits rather than presses | T69 | Both entropy audits agreed the health check cannot catch a clean-but-shallow RNG -- 0 detections in 200,000 at every sample size, which is the Coldcard shape exactly -- and that user entropy is the only layer that covers it. Dice give a number that is arithmetic instead of modelled: `log2(6)` credited as `floor(n x 2585/1000)`, so the figure on screen can never exceed the real one. 50 rolls = 129 bits, the same 128-bit gate taps have always had, and rolls and taps spend against one budget so they compose. Dice and taps are separate ledgers: the presses made *while entering* rolls are hashed in and credited zero, because charging one physical act to two ledgers would stop the total being a floor. **Physical dice only**, said on screen every frame rather than behind a one-time OK -- a phone app is an unauditable PRNG on a networked device, so a compromised phone yields an attacker-chosen seed while leaving the user more confident in a weaker one |
| ~~T69~~ ✅ | **Temporary seed**: type a phrase, hold it in RAM for the session, store nothing. The mode SeedSigner is built around, offered here as a menu action rather than a global toggle — a toggle leaves stored wallets and a typed seed both present with nothing on screen settling which one signs. Every at-rest finding in `docs/AUDIT-SECRETS.md` is an attack on a stored seed; this has none. Paid for with two costs the user reads first: the PIN guards nothing at rest, and a reboot or flat battery loses the seed | T42 | held in `state.mnemonic` beside the passphrase, on the passphrase's own lifetime; cleared by lock, auto-lock, wallet switch, wipe and power loss; auto-lock defaults to 30 min for this mode only; `sim/test_temp_seed.c` asserts zero NVS writes against the real vault |
