...
| # | Where | Issue |
|---|-------|-------|
| ~~a~~ ✅ | `src/ui.c` | **Fixed (T7).** Errors now live in their own `address_error` field and `address_valid()` is the single gate every consumer asks, so an error can no longer render as a truncated address. The finding understated it: `screen_qr_code_render` tested only for empty and `"No wallet"`, so `"Addr failed"`, `"Derive failed"`, `"Unlock failed"` and `"Select failed"` were **encoded into a QR code and presented as a receive address**. The passphrase-confirmation screen was worse still — it asks the user to recognise an address, and drew blank lines under "Match your record" when derivation failed, inviting them to accept a wallet that never derived. Covered by 4 groups in `sim/test_ui.c`; restoring the old guard reproduces the QR bug in the test output. |
