 ...
 * Copyright (c) 2017 Richard Moore (MIT License)
...
/* BitBucket for bit manipulation */
typedef struct {
...
static void bb_appendBits(BitBucket *bb, uint32_t val, uint8_t length) {
    ...
    bb->bitOffsetOrWidth = offset;
}

static void bb_setBit(BitBucket *bb, uint8_t x, uint8_t y, bool on) {
    uint32_t offset = y * bb->bitOffsetOrWidth + x;
    uint8_t mask = 1 << (7 - (offset & 0x07));
    if (on) {
        bb->data[offset >> 3] |= mask;
    } else {
        bb->data[offset >> 3] &= ~mask;
    }
}

static bool bb_getBit(BitBucket *bb, uint8_t x, uint8_t y) {
    uint32_t offset = y * bb->bitOffsetOrWidth + x;
    return (bb->data[offset >> 3] & (1 << (7 - (offset & 0x07)))) != 0;
}

static void bb_invertBit(BitBucket *bb, uint8_t x, uint8_t y, bool invert) {
    uint32_t offset = y * bb->bitOffsetOrWidth + x;
    uint8_t mask = 1 << (7 - (offset & 0x07));
    bool on = ((bb->data[offset >> 3] & (1 << (7 - (offset & 0x07)))) != 0);
    if (on ^ invert) {
        bb->data[offset >> 3] |= mask;
    } else {
        bb->data[offset >> 3] &= ~mask;
...
static void drawFinderPattern(BitBucket *modules, BitBucket *isFunction,
    ...
    for (int8_t i = -4; i <= 4; i++) {
        for (int8_t j = -4; j <= 4; j++) {
            uint8_t dist = max_val(abs(i), abs(j));
...
static void drawAlignmentPattern(BitBucket *modules, BitBucket *isFunction,
    ...
    for (int8_t i = -2; i <= 2; i++) {
        for (int8_t j = -2; j <= 2; j++) {
            setFunctionModule(modules, isFunction, x + j, y + i, max_val(abs(i), abs(j)) != 1);
        }
    }
}
